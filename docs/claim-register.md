...
## J. Capability Intelligence — worked-example provenance

The `/capability-intelligence/` "A brief becomes a defensible squad" walkthrough is an **illustrative example on synthetic data**. The coloured provenance tags (Generated / Derived / Illustrative future) were removed from the page for marketing cleanliness; provenance is tracked here instead. Status of each step:

| Step | Provenance | On the live prototype? |
|---|---|---|
| Clarified outcomes & critical capabilities | Generated by the prototype | Yes |
| Evidence discovered beyond CVs | Generated by the prototype | Yes |
| Proposed squad coverage | Derived from prototype data | Yes |
| Gaps & key-person risk | Derived from prototype data | Yes |
| Collaboration-optimised composition & growth plan | **Illustrative future workflow** | **No — roadmap.** Page marks it "on the roadmap"; must never read as live. |

Gate: the page may surface matches, gaps and collaborator links (built); composition optimisation, growth plans and a client-ready pack are **roadmap** and stay labelled as such.
