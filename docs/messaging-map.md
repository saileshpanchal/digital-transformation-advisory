...
- Listing page intro + per-post (title/excerpt/body). Flagship: DBB paper; Cross-border ("the real prize is the deposit"); **Data Products for Agentic Banking** (2026-06-16 — TFM as external anchor; the governed data product is the missing unit; links to `/composable-bank/data-products/`); **The Self-Diagnosing Bank** (2026-06-16 — operational resilience as architecture; PS21/3/DORA; blast-radius; links to `/composable-bank/resilience/`); **Independent Architecture, Proven Primitives** (2026-06-17 — the platform decision comes second; executable target architecture proves the hard primitives first; bank-pain framing; carries the three-legs spine once at the close; links Platform / Approach / Capability Intelligence).

## Positioning thread (subtle category signal) [cross-cutting]
> Governed in `claim-register.md` › *Positioning (DTA internal)*. Audience: senior people at major consulting firms (forward-and-skim test). **Subtle in tone, clear in claim; aimed at bank pain, never at any advisor's deliverable.**
- **Category line** (forwardable): Platform hero lede + a one-line echo on Home near the mission. "Most banks choose a platform before anyone has proven the hard parts work. We define an independent target architecture and prove those parts first, so the platform conforms to the architecture, not the other way round."
- **Evidence beat** (Platform, "Credible because it is labelled"): "Vendors sell a platform as the answer; regulators ask for evidence, not assertions… an executable target architecture, a claim register… and the decision records behind it. Independent of any platform, and honest about what is demonstrated versus blueprint."
- **Three-legs spine** (one quiet line each on Platform / Approach / Capability Intelligence, each cross-linking the others): "Three parts of one capability: the target architecture (what good looks like, proven), the operating model to build it, and the capability intelligence to staff it."
- **"Questions this architecture answers"** (Platform, post-hero): CISO/CDO/CIO/CEO worries at **stakeholder-pain level, not mechanism level** (name the concern, not the internal answer).
- **Discipline:** no superlatives, no pricing, no "top 1%"; codenames never public; "demonstrated, not production" holds.
